...
// WriteToAPIGatewayProxyResponse writes the problem detail to the API Gateway
// response as JSON.
// The content type is set to application/problem+json; charset=utf-8.
// The status code will be set to both ProblemDetail.Status and
// events.APIGatewayProxyResponse.
//
// If the problem detail is invalid, an error is returned.
func WriteToAPIGatewayProxyResponse(r *events.APIGatewayProxyResponse, pd ProblemDetailer, code int) error {
	r.StatusCode = code
	if err := pd.Validate(); err != nil {
		return fmt.Errorf("WriteToAPIGatewayProxyResponse: %w", err)
	}
	if r.Headers == nil {
		r.Headers = make(map[string]string)
	}
	r.Headers["Content-Type"] = "application/problem+json; charset=utf-8"
